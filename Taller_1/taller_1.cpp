...
double taller(vector<double> limites, double particiones, double *error,double * recursiones, double xrAnterior, double* errorMasUno)
{
    ...
    *recursiones=(*recursiones)+1;//SUMAR EN 1 EL NUMERO DE ITERACIONES(REECURSIONES)
    ...
    {
        errorPorcentual = ((limites[0] + limites[1])/2)*100.00000;
        cout<<"ERROR PORCENTUAL ---"<<setprecision(5)<<errorPorcentual<<" con "<<seccion<<" "<<xrAnterior<<"--"<<seccion + xrAnterior<<endl; 
    }
    //-----------------------------------------------------------------------
    /*
    ...
    */
    for(int i=0; i<Particiones.size(); i++)//-----------------------------------------------------
    {
        ...
        /*if(lol<0)//ESTO LO COMENTÓ BRIAM
        {
            lol*=-1;
        }*/
    ...
    for(int i=1; i<Particiones.size(); i++)
    {
        ...
        else
        {
            ...
            if(*recursiones!=1)
            {
             ...
             /*if(xrAnterior!=0.00000)
             {
            errorPorcentual = ((seccion + xrAnterior)/seccion)*100.00000; 
             }
            xrAnterior=seccion;*/
